...
def boss_fight(score_value):
    start = time.time()
    enemy_laser_start = time.time()
    enemy_cannon_start = time.time()
    ...
    enemy_laserY_change, max_enemy_lasers = 5, 10
    ...
    # Enemy Cannon
    enemy_cannonIMG = pygame.image.load("ART/majorlazer.png")
    enemy_cannonX = (X / 2) - 50
    enemy_cannonY = 95
    enemy_cannon_state = "ready"
    ...
    def show_score(x, y):
        boss_health_score = font.render("Boss Health: " + str(boss_health), True, (255, 0, 0))
        win.blit(boss_health_score, (x, y))
        return
    ...
    # boss function to draw the boss to the window.
    def boss(x, y):
    ...
    def enemy_cannon(x, y):
        win.blit(enemy_cannonIMG, (x, y))
        return
    ...
    while run:
        ...
        if enemy_laser_timer >= 3:
            ...
            for i in range(max_enemy_lasers):
                # print("enemy Fire")
                enemy_laserX[i] = random.randint(1, (X - 65))
                enemy_laserY[i] = 95
                enemy_laser_state[i] = "fired"
                enemy_laser_start = time.time()

        # Timer for when the enemy main cannon
        enemy_cannon_timer = (time.time() - enemy_cannon_start)
        if enemy_cannon_timer >= 7:
            enemy_cannon(enemy_cannonX, enemy_cannonY)
            enemy_laser_sound.play()
            # print("enemy cannon fire")
            enemy_cannon_state = "fired"
            enemy_cannon_start = time.time()
        ...
        for left_shotgun_laser in range(left_max_shotgun_lasers):
            if left_shotgunY_laser[left_shotgun_laser] <= 95:
        ...
        for center_shotgun_laser in range(center_max_shotgun_lasers):
            if center_shotgunY_laser[center_shotgun_laser] <= 95:
                explosion_sound = mixer.Sound('SFX/explosion.wav')
                explosion_sound.play()
        ...
        for right_shotgun_laser in range(right_max_shotgun_lasers):
            if right_shotgunY_laser[right_shotgun_laser] <= 95:
                explosion_sound = mixer.Sound('SFX/explosion.wav')
                explosion_sound.play()
        ...
        # Enemy Cannon fire and reset
        enemy_cannon_end_timer = (time.time() - enemy_cannon_start)
        if enemy_cannon_state == "fired":
            enemy_cannon(enemy_cannonX, enemy_cannonY)
            if playerX > enemy_cannonX - 50 and playerX < enemy_cannonX + 100:
                break
        if enemy_cannon_end_timer >= 3:
            enemy_cannon_state = "ready"
        ...
        # If boss health reaches zero end the game
        if boss_health <= 0:
            break
